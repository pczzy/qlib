...
from model_provenance import expected_fingerprint

# --- 常量定义：解决 Magic Strings 问题 ---
PARAMS_FILE = "params.pkl"
SIG_ANALYSIS_DIR = "sig_analysis"
REQUIRED_ARTIFACTS = [PARAMS_FILE, SIG_ANALYSIS_DIR, "training_provenance"]
...
class ModelCLI:
    ...
    def _is_valid_recorder(self, recorder):
        ...
        if not artifacts or not all(f in artifacts for f in REQUIRED_ARTIFACTS):
            return False
        # Historical rolling cohorts stay in MLflow for traceability, but only
        # the five windows anchored to the active provider's last trading day
        # may participate in today's ensemble/selection.
        try:
            task = recorder.load_object("task")
            segments = task["dataset"]["kwargs"]["segments"]
            expected = self._current_segments()
            normalized = {
                key: tuple(str(value) for value in segments[key])
                for key in ("train", "valid", "test")
            }
            if normalized not in expected:
                return False
            provenance = recorder.load_object("training_provenance")
            if provenance.get("recorder_fingerprint_sha256") != expected_fingerprint(
                task, self.kwargs["provider_uri"]
            ):
                return False
        except Exception:
            return False
    ...
    def _current_segments(self):
        ...
        return [
            generate_qlib_segments(12 * i, end_date_str=data_end)
            for i in range(1, 6)
        ]
